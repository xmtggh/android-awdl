...
int neighbor_add(int ifindex, const struct ether_addr *eth, const struct in6_addr *in6) {
    log_error("function :: %s  line :: %d",__FUNCTION__,__LINE__);
	if (fork() <= 0) {
		char eth_str[18];
		...
		slog_error(in6scope_str, "%s%%%s", inet_ntop(AF_INET6, (void *) in6, in6_str, sizeof(in6_str)),
