...
#define APPLE_P2P_SYNC_THRESHOLD 3

int apple_p2p_handle_sync_params_tlv(struct apple_p2p_peer *src, const struct buf *val, struct apple_p2p_state *state, uint64_t now) {
	uint16_t aw_counter_master;
	uint16_t time_to_next_aw_master;
	int64_t sync_err_tu;

	if (!apple_p2p_election_is_sync_master(&state->election, &src->addr))
	...
	sync_err_tu = apple_p2p_sync_error_tu(now, time_to_next_aw_master, aw_counter_master, &state->sync);
	if (sync_err_tu > APPLE_P2P_SYNC_THRESHOLD || sync_err_tu < -APPLE_P2P_SYNC_THRESHOLD) {
		state->sync.meas_err++;
		log_trace("Sync error %d TU (%.02f %%)", sync_err_tu, state->sync.meas_err * 100.0 / state->sync.meas_total);
	}
	apple_p2p_sync_update_last(now, time_to_next_aw_master, aw_counter_master, &state->sync);
...
int apple_p2p_handle_chanseq_tlv(struct apple_p2p_peer *src, const struct buf *val,
                            struct apple_p2p_state *state __attribute__((unused))) {
	uint8_t count;
	uint8_t encoding;
	uint8_t duplicate_count;
	uint8_t step_count;
	uint16_t fill_channel;
	struct apple_p2p_chan list[APPLE_P2P_CHANSEQ_LENGTH];
	int size;

	/* read and sanity check, we do not support any other configuration */
	READ_U8(val, 0, &count);
	if (count + 1 != APPLE_P2P_CHANSEQ_LENGTH)
	...
	size = apple_p2p_chan_encoding_size(encoding);
	if (size < 1)
		return RX_UNEXPECTED_VALUE;

	for (int i = 0, offset = 6; i < APPLE_P2P_CHANSEQ_LENGTH; i++, offset += size)
	...
	if (memcmp(src->sequence, list, sizeof(list))) {
		log_debug("peer %s (%s) changed channel sequence to %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
		          ...
		          apple_p2p_chan_num(list[0], encoding), apple_p2p_chan_num(list[1], encoding),
		          apple_p2p_chan_num(list[2], encoding), apple_p2p_chan_num(list[3], encoding),
		          apple_p2p_chan_num(list[4], encoding), apple_p2p_chan_num(list[5], encoding),
		          apple_p2p_chan_num(list[6], encoding), apple_p2p_chan_num(list[7], encoding),
		          apple_p2p_chan_num(list[8], encoding), apple_p2p_chan_num(list[9], encoding),
		          apple_p2p_chan_num(list[10], encoding), apple_p2p_chan_num(list[11], encoding),
		          apple_p2p_chan_num(list[12], encoding), apple_p2p_chan_num(list[13], encoding),
		          apple_p2p_chan_num(list[14], encoding), apple_p2p_chan_num(list[15], encoding));
...
int apple_p2p_handle_election_params_tlv(struct apple_p2p_peer *src, const struct buf *val,
                                    struct apple_p2p_state *state __attribute__((unused))) {
...
int apple_p2p_handle_arpa_tlv(struct apple_p2p_peer *src, const struct buf *val,
                         struct apple_p2p_state *state __attribute__((unused))) {
...
int apple_p2p_handle_data_path_state_tlv(struct apple_p2p_peer *src, const struct buf *val,
                                    struct apple_p2p_state *state __attribute__((unused))) {
	uint16_t flags;
	int offset = 0;
	...
	if (flags & APPLE_P2P_DATA_PATH_FLAG_COUNTRY_CODE) {
		READ_BYTES_COPY(val, offset, (uint8_t *) src->country_code, 3);
		offset += 3;
	}
	if (flags & APPLE_P2P_DATA_PATH_FLAG_SOCIAL_CHANNEL_MAP) {
		READ_LE16(val, offset, NULL /* supported social channels */);
		offset += 2;
	}
	if (flags & APPLE_P2P_DATA_PATH_FLAG_INFRA_INFO) {
		READ_ETHER_ADDR(val, offset, NULL /* BSSID */);
		offset += 6;
		READ_LE16(val, offset, NULL /* channel */);
		offset += 2;
	}
	if (flags & APPLE_P2P_DATA_PATH_FLAG_INFRA_ADDRESS) {
		READ_ETHER_ADDR(val, offset, &src->infra_addr);
		offset += 6;
	}
	if (flags & APPLE_P2P_DATA_PATH_FLAG_APPLE_P2P_ADDRESS) {
		READ_ETHER_ADDR(val, offset, NULL /* APPLE_P2P address */);
		offset += 6;
	}
...
int apple_p2p_handle_version_tlv(struct apple_p2p_peer *src, const struct buf *val,
                            struct apple_p2p_state *state __attribute__((unused))) {
...
int apple_p2p_handle_tlv(struct apple_p2p_peer *src, uint8_t type, const struct buf *val,
	...
	switch (type) {
		case APPLE_P2P_SYNCHRONIZATON_PARAMETERS_TLV:
			return apple_p2p_handle_sync_params_tlv(src, val, state, tsft);
		case APPLE_P2P_CHAN_SEQ_TLV:
			return apple_p2p_handle_chanseq_tlv(src, val, state);
		case APPLE_P2P_ELECTION_PARAMETERS_TLV:
			return apple_p2p_handle_election_params_tlv(src, val, state);
		case APPLE_P2P_ELECTION_PARAMETERS_V2_TLV:
			return apple_p2p_handle_election_params_v2_tlv(src, val, state);
		case APPLE_P2P_ARPA_TLV:
			return apple_p2p_handle_arpa_tlv(src, val, state);
		case APPLE_P2P_DATA_PATH_STATE_TLV:
			return apple_p2p_handle_data_path_state_tlv(src, val, state);
		case APPLE_P2P_VERSION_TLV:
			return apple_p2p_handle_version_tlv(src, val, state);
		case APPLE_P2P_SYNCTREE_TLV: /* seems to be buggy, not used in our election process */
			/* fall through */
		default:
			//log_trace("apple_p2p: not handling %s (%u)", apple_p2p_tlv_as_str(type), type);
			return RX_IGNORE;
	}
}

int apple_p2p_parse_action_hdr(const struct buf *frame) {
	const struct apple_p2p_action *af;
	int valid;

	READ_BYTES(frame, 0, NULL, sizeof(struct apple_p2p_action));

	af = (const struct apple_p2p_action *) buf_data(frame);

	valid = af->category == IEEE80211_VENDOR_SPECIFIC &&
	        !memcmp(&af->oui, &APPLE_P2P_OUI, sizeof(APPLE_P2P_OUI)) &&
	        af->type == APPLE_P2P_TYPE &&
	        af->version == APPLE_P2P_VERSION_COMPAT;

	if (valid && (af->subtype == APPLE_P2P_ACTION_PSF || af->subtype == APPLE_P2P_ACTION_MIF))
		return af->subtype;
...
int apple_p2p_rx_action(const struct buf *frame, signed char rssi, uint64_t tsft,
	...
	int subtype;
    log_trace("apple_p2p_action: ");
	(void) dst; /* TODO ignore destination address for now, could be used to mitigate desynchronization attack */

	subtype = apple_p2p_parse_action_hdr(frame);
	if (subtype < 0) {
		log_trace("apple_p2p_action: not an action frame"); /* could be block ACK */
		return RX_IGNORE;
	}
	buf_strip(frame, sizeof(struct apple_p2p_action));

	if (state->filter_rssi) {
		status = apple_p2p_peer_get(state->peers.peers, src, NULL);
	...
	/* Update peer table */
	status = apple_p2p_peer_add(state->peers.peers, src, tsft, state->peer_cb, state->peer_cb_data);
	if (status < 0) {
//		log_warn("apple_p2p_action: could not add peer: %s (%d)", ether_ntoa(src), status);
		return RX_IGNORE;
	}
	status = apple_p2p_peer_get(state->peers.peers, src, &peer);
	if (status < 0) {
//		log_warn("apple_p2p_action: could not find peer: %s (%d)", ether_ntoa(src), status);
		return RX_IGNORE; /* FIXME happens sometimes, not sure why */
	}

	log_trace("apple_p2p_action: receive %s from %s (rssi %d)", apple_p2p_frame_as_str(subtype), ether_ntoa(&peer->addr), rssi);

	while ((len = read_tlv(frame, 0, &tlv_type, &tlv_len, &tlv_value)) > 0) {
		const struct buf *tlv_buf = buf_new_const(tlv_value, tlv_len);
		int result = apple_p2p_handle_tlv(peer, tlv_type, tlv_buf, state, tsft);
		if (state->tlv_cb)
			state->tlv_cb(peer, tlv_type, tlv_buf, state, state->tlv_cb_data);
		buf_free(tlv_buf);
		if (result < 0) {
//			log_warn("apple_p2p_action: parsing error %s", apple_p2p_tlv_as_str(tlv_type));
	...
	if (buf_len(frame) > 0) {
//		log_debug("apple_p2p_action: unexpected bytes (%d) at end of frame", buf_len(frame));
		return RX_UNEXPECTED_FORMAT;
	}

	if (subtype == APPLE_P2P_ACTION_MIF)
		peer->sent_mif = 1;

	/* update peer info after parsing all TLVs */
	apple_p2p_peer_add(state->peers.peers, src, tsft, state->peer_cb, state->peer_cb_data);
...
int apple_p2p_valid_llc_header(const struct buf *frame) {
	...
	if (llc.dsap != 0xaa || llc.ssap != 0xaa || llc.control != 0x03 ||
	    llc.pid != APPLE_P2P_LLC_PROTOCOL_ID) {
...
int apple_p2p_rx_data(const struct buf *frame, struct buf ***out, const struct ether_addr *src,
                 const struct ether_addr *dst, struct apple_p2p_state *state) {
	uint16_t ether_type;
	int offset = 0;

	log_trace("apple_p2p_data: receive from %s", ether_ntoa(src));
	state->stats.rx_data++;

	if (apple_p2p_peer_get(state->peers.peers, src, NULL) != PEERS_OK)
		return RX_IGNORE_PEER;

	if (!apple_p2p_valid_llc_header(frame))
		return RX_UNEXPECTED_FORMAT;
	buf_strip(frame, sizeof(struct llc_hdr));

	if ((unsigned int) buf_len(frame) < sizeof(struct apple_p2p_data)) {
		log_warn("apple_p2p_data: frame too short");
		return RX_TOO_SHORT;
	}

	/* create ethernet frame */
	read_be16(frame, 6, &ether_type);
	buf_strip(frame, sizeof(struct apple_p2p_data));
...
int apple_p2p_rx_data_amsdu(const struct buf *frame, struct buf ***out, const struct ether_addr *src __attribute__((unused)),
                       const struct ether_addr *dst __attribute__((unused)), struct apple_p2p_state *state) {
	...
	while (buf_len(frame) > 0) {
		...
		err = apple_p2p_rx_data(subframe, out, &src_a, &dst_a, state);
		buf_free(subframe);
		if (err < 0)
			return err;
...
int apple_p2p_rx(const struct buf *frame, struct buf ***data_frame, struct apple_p2p_state *state) {
	...
	switch (fc & (IEEE80211_FCTL_FTYPE | IEEE80211_FCTL_STYPE)) {
		case IEEE80211_FTYPE_MGMT | IEEE80211_STYPE_ACTION:
			return apple_p2p_rx_action(frame, rssi, tsft, from, to, state);
		case IEEE80211_FTYPE_DATA | IEEE80211_STYPE_DATA | IEEE80211_STYPE_QOS_DATA:
			...
			if (qosc & IEEE80211_QOS_CTL_A_MSDU_PRESENT)
				return apple_p2p_rx_data_amsdu(frame, data_frame, from, to, state);
			/* else fall through */
		case IEEE80211_FTYPE_DATA | IEEE80211_STYPE_DATA:
			return apple_p2p_rx_data(frame, data_frame, from, to, state);
		default:
